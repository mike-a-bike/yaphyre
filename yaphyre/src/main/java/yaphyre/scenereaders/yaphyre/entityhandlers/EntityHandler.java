...
public abstract class EntityHandler<T extends IdentifiableObject<?>> {
	...
	/**
	 * The method which decodes the entity. This method gets a selected match based on the returned XPath selector by the
	 * {@link #getXPath()} method. The {@link Match} represents a single instance of the selection result. As supporting
	...
	/**
	 * If the referenced entities are restricted to specific parent tags, this method returns a list of these tags. If the
	 * usage is not restricted, an empty {@link List} is returned.
	 *
	 * @return A list with valid parents. If no restriction applies, an empty {@link List} is returned.
	 */
